	...
	if (gpio_is_valid(p_sar->dts_info.irq_gpio)) {
		...
		if (ret) {
			...
			if (ret != 0) {
				...
				ret = AW_OK;
				p_sar->intrrupt_init_state = AW_TRUE;
				AWLOGI(p_sar->dev,
					"%d", p_sar->ret_val );
			}
		}
...
static int32_t aw_sar_input_init_comm(struct aw_sar *p_sar)
{
	...
	for (i = 0; i < p_sar->p_sar_para->ch_num_max; i++) {
		...
		if ((p_sar->dts_info.channel_use_flag >> i) & 0x01) {
			...
			__set_bit(EV_ABS, p_sar->channels_arr[i].input->evbit);
...
#ifdef CONFIG_USE_POWER_SUPPLY_ONLINE
	AWLOGI(p_sar->dev, "%s is %s", psy->desc->name,
...
static int aw_sar_ps_notify_init(struct aw_sar *p_sar)
{
	...
	if (psy) {
		...
		if (ret) {
			AWLOGE(p_sar->dev, "psy get property failed rc=%d",
				ret);
